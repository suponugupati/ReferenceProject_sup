...
public class Utils {
	...
	{
		WebDriverWait wait = new WebDriverWait(LocalDriverManager.getDriver(), 20);  
		wait.until(ExpectedConditions.visibilityOfElementLocated(by));
	}
	...
	public boolean verifyElementNotPresent(By element, String value){
		boolean flag;
		try{
			List<WebElement> elements = LocalDriverManager.getDriver().findElements(element);
			if(elements.size()==0)
				flag= true;
			else
				flag= false;
	...
	public static void scroll(WebDriver driver, int x, int y)
	{
		...
		try {
			...
			String code="window.scroll("+(x)+"," +
					(y)+");";
			((JavascriptExecutor)LocalDriverManager.getDriver()).executeScript(code, x, y);
	...
	public boolean click( WebElement elementLocator , String value) throws Throwable {
		...
		try {
			final WebDriverWait wait = new WebDriverWait(LocalDriverManager.getDriver(), 60);
	...
	public boolean Jsclick( WebElement elementLocator , String value) throws Throwable {
		boolean status = false;
		try {
			((JavascriptExecutor)LocalDriverManager.getDriver()).executeScript("arguments[0].click();", elementLocator);
			status = true;
			return true;
		} catch (final Exception e) {
			status = false;
	...
	public static void doubleClick(WebElement element) {
		...
		((JavascriptExecutor) LocalDriverManager.getDriver()).executeScript("arguments[0].scrollIntoView();", element);
	...
	{
		try{
			WebDriverWait wait = new WebDriverWait(LocalDriverManager.getDriver(), 6);
	...
	{
		String url = LocalDriverManager.getDriver().getCurrentUrl();
	...
	public void waitForPageLoad() {
		...
		wait.until(new Function<WebDriver, Boolean>() {
			public Boolean apply(WebDriver driver) {
				return String
						.valueOf(((JavascriptExecutor) LocalDriverManager.getDriver()).executeScript("return document.readyState"))
						.equals("complete");
			}
		});
	}
	...
	public boolean waitTime() {
		...
		Long loadTime;
		try {
			loadTime = (Long) ((JavascriptExecutor) LocalDriverManager.getDriver())
	...
	public void switchToNewWindowFromCurrentWindow() throws Throwable {
		try {
			ArrayList<String> tabs = new ArrayList<String> (LocalDriverManager.getDriver().getWindowHandles());
			LocalDriverManager.getDriver().switchTo().window(tabs.get(1));
		} catch (final Exception e) {
	...
	public void switchToParentWindow() throws Throwable {
		try {
			ArrayList<String> tabs = new ArrayList<String> (LocalDriverManager.getDriver().getWindowHandles());
			LocalDriverManager.getDriver().switchTo().window(tabs.get(0));
		} catch (final Exception e) {
